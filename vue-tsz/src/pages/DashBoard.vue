        ...
        <el-menu :collapse="hideNavigation">
	        <transition name="el-fade-in-linear">
		        <el-menu-item index="0" v-if="navigationLogo" style="padding: 2px;height: 100%;">
			        <el-image src="https://wpimg.wallstcn.com/69a1c46c-eb1c-4b46-8bd4-e9e686ef5251.png" fit="fill"></el-image>
		        </el-menu-item>
	        </transition>
          ...
          <el-submenu  index="2">
            ...
            <el-menu-item-group>
              <el-menu-item index="2-1" @click="changeNavigation('普通用户','/admin/dashboard/usermanager/1', '2-1')">普通用户</el-menu-item>
              <el-menu-item index="2-2" @click="changeNavigation('管理员账户','/admin/dashboard/usermanager/2', '2-2')">管理员用户</el-menu-item>
            </el-menu-item-group>
          </el-submenu>
          <el-submenu index="3">
	          <template slot="title">
            ...
            <el-menu-item-group>
              <el-menu-item index="3-1" @click="changeNavigation('系统订单','/admin/dashboard/bookmanager/1', '3-1')">系统订单</el-menu-item>
              <el-menu-item index="3-2" @click="changeNavigation('账户订单','/admin/dashboard/bookmanager/2', '3-2')">账户订单</el-menu-item>
	            <el-menu-item index="3-3" @click="changeNavigation('订单列表','/admin/dashboard/bookmanager/3', '3-3')">订单列表</el-menu-item>
            </el-menu-item-group>
          </el-submenu>
		      <el-menu-item index="4" @click="changeNavigation('系统设置','/admin/dashboard/settings', '4-1')">
	    ...
	    <el-container>
		    <el-main style="padding: 0">
			    <el-row style="width: 100%">
				    <el-col :span="1" style="text-align:center;line-height: 60px;background-color: #eeeeee;">
				      <el-button type="text"><i class="el-icon-s-fold" style="color: black;font-size: 16px;" v-if="!hideNavigation" @click="hideNavigation=true"></i></el-button>
					    <el-button type="text"><i class="el-icon-s-unfold" style="color: black;font-size: 16px;" v-if="hideNavigation" @click="hideNavigation=false"></i></el-button>
				    </el-col>
				    <el-col :span="15">
					    <el-header style="text-align: right;line-height: 60px; background-color: #eeeeee" >
								<el-breadcrumb separator="/" style="line-height: 60px;">
									<el-breadcrumb-item v-for="(item,index) in currentNavigation" :to="{path: item.url}" v-bind:key="index"> {{item.name}}</el-breadcrumb-item>
								</el-breadcrumb>
					    </el-header>
				    </el-col>
				    <el-col :span="8" style="background-color: #eeeeee; line-height: 60px; font-size: 16px; color: black" class="fr">
					    <el-row :gutter=10>
						    <el-col :span="14"><el-input placeholder="搜索" suffix-icon="el-icon-search"></el-input></el-col>
						    <el-col :span="2"><a><i class="el-icon-circle-plus-outline"></i></a></el-col>
						    <el-col :span="2"><a><i class="glyphicon glyphicon-text-size"></i></a></el-col>
						    <el-col :span="2"><a><i class="fa fa-language"></i></a></el-col>
						    <el-col :span="3" style="padding: 10px 0; height: 60px">
							    <el-dropdown>
							      <el-avatar class="el-dropdown-link" :size="35" src="https://cube.elemecdn.com/0/88/03b0d39583f48206768a7534e55bcpng.png" style="line-height: 30px"></el-avatar>
			    ...
			    <el-button v-if="!rightDrawerOpened" @click="rightDrawerOpened=true" type="primary" style="position: fixed;top: 200px; right: 0;z-index: 1000">
				    <i class="el-icon-setting"></i>
			    </el-button>
			    <el-button v-else type="primary" style="position: fixed;top: 200px; right: 15%;z-index: 2034">
			    ...
			    <el-container style="height: 1000px">
				    <el-main class="height-100" v-if="currentPathIndex=='1-1'">
					    主页
				    </el-main>
						    ...
						    <el-table-column type="expand">
							    <template slot-scope="props">
							      <el-form label-position="left" class="table-expand">
								      <el-row>
									      <el-col :span=8><el-form-item label="用户名"><span>{{props.row.userName }}</span></el-form-item></el-col>
									      <el-col :span=8><el-form-item label="昵称"><span>{{props.row.userNickName}}</span></el-form-item></el-col>
									      <el-col :span=8><el-form-item label="密码"><span>{{props.row.userPassword}}</span></el-form-item></el-col>
								      </el-row>
								      <el-row>
									      <el-col :span=8><el-form-item label="用户类别"><span>{{props.row.userOrgID}}</span></el-form-item></el-col>
									      <el-col :span=8><el-form-item label="性别"><span>{{props.row.userSex}}</span></el-form-item></el-col>
									      <el-col :span=8><el-form-item label="电话"><span>{{props.row.userTel}}</span></el-form-item></el-col>
								      </el-row>
								      <el-row>
									      <el-col :span=8><el-form-item label="邮箱"><span>{{props.row.userEmail}}</span></el-form-item></el-col>
									      <el-col :span=8><el-form-item label="创建时间"><span>{{props.row.userJoinDate}}</span></el-form-item></el-col>
									      <el-col :span="8"><el-form-item label="最近登录"><span>{{props.row.userLastLogin}}</span></el-form-item></el-col>
								      </el-row>
							      </el-form>
							    </template>
						    </el-table-column>
								    ...
								    <el-row>
									    <el-col :span="8">
										    <el-form-item label="服务器名称">
											    <el-input v-model="systemDetails.serviceName" :disabled="!systemOperation.operatorEdit"></el-input>
										    </el-form-item>
										    <el-form-item label="服务器IP">
											    <el-input v-model="systemDetails.serviceIP" :disabled="!systemOperation.operatorEdit"></el-input>
										    </el-form-item>
										    <el-form-item label="启用">
											    <el-switch v-model="systemDetails.serviceEnable" active-color="#13ce66" inactive-color="#ff4949" :disabled="!systemOperation.operatorEdit"></el-switch>
										    </el-form-item>
										    <el-form-item label="服务器端口">
											    <el-input v-model="systemDetails.servicePort" :disabled="!systemOperation.operatorEdit"></el-input>
										    </el-form-item>
										    <el-form-item label="服务器证书">
											    <el-input type="file" v-model="systemDetails.serviceCA" :disabled="!systemOperation.operatorEdit"></el-input>
										    </el-form-item>
										    <el-form-item label="服务允许协议">
											    <el-row :gutter=10>
												    <el-col :span="18">
												      <el-input type="text" v-model="systemDetails.serviceProtocol" :disabled="!systemOperation.operatorEdit"></el-input>
												    </el-col>
												    <el-col :span="4">
													    <el-button type="text" @click="systemOperation.showServiceProtocol=true">预览协议</el-button>
													    <el-dialog title="用户协议" :visible.sync="systemOperation.showServiceProtocol" width="39%" center :open="getProtocolContent()">
														    <pre>{{systemOperation.systemProtocolContent}}</pre>
														    <span slot="footer" class="dialog-footer">
															    <el-button @click="systemOperation.showServiceProtocol = false">取 消</el-button>
															    <el-button type="primary" @click="systemOperation.showServiceProtocol = false">确 定</el-button>
                                </span>
													    </el-dialog>
												    </el-col>
											    </el-row>
										    ...
										    <el-row :gutter=8>
											    ...
											    <el-button type="reset" @click="systemOperation.operatorEdit=false; systemDetails=systemDetails_bak;">撤销</el-button>
										    </el-row>
									    </el-col>
								    </el-row>
							    </el-form>
    ...
    async getProtocolContent() {
      try {
        ...
        if(retData.length == 0){
          this.$message({
            type: 'warning',
            message: 'request serviceProtocol' + this.systemDetails.serviceProtocol +' error.'
          });
        }else
          this.systemOperation.systemProtocolContent = retData.data;
      }catch(err){
        this.$message({
	        type: 'warning',
...
<style scoped>
	...
	.setting-row{
		padding: 10px;height: 60px;font-size: 16px;
	}
	.setting-col{
