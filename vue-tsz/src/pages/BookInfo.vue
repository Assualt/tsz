    ...
    <el-main style="width: 100%; padding: 20px 10%">
      ...
      <el-row style="padding: 10px 8%" :guater="10">
        <el-col :span="5">
          ...
          >
            <a href="#">
              <el-image
                src="/static/upload/陪孩子终生成长(樊登2020新书).jpg"
                style="height: 268px; width: 200px"
              ></el-image>
            </a>
          </div>
          <el-link>收藏</el-link>
        ...
        <el-col :span="14" :offset="1">
          ...
          <p :title="currentBook.description" class="book-description">
            {{ currentBook.description }}
          </p>
          <el-row style="line-height: 30px; background-color: #f5f5f5; padding: 5px">
            <el-col :span="2">售价:</el-col>
            <el-col :span="3" style="font-size: 24px; font-weight: bold; color: red"
              >&yen;{{ currentBook.price.now }}
            </el-col>
            ...
            <el-col style="line-height: 40px"
              >在
              <el-link :href="currentBook.rank.url" :underline="false">{{
                currentBook.rank.typename
              }}</el-link>
            ...
            <el-col>
              ...
              <el-rate
                v-model="currentBook.rate.score"
                show-score
                text-color="#ff9900"
                class="fr"
          ...
          <el-row style="padding: 5px; line-height: 40px">
            ...
            <el-col :span="3">{{
              currentBook.storage_number < 10 ? "库存紧张" : currentBook.storage_number
            }}</el-col>
            <el-col :span="6">
            ...
            <el-col :span="20"
              >由"当当"发货，并提供售后服务。
              23:15前完成下单，预计明天<strong>(8月2日)</strong>可送达</el-col
            >
            <el-col :span="3">数量:</el-col>
            <el-col :span="4">
              <el-input-number
                v-model="currentBook.select_cnt"
                :min="1"
                :max="currentBook.storage_number"
                size="small"
              ></el-input-number>
            </el-col>
        ...
        <el-col :span="4">
          ...
          <el-col style="padding-left: 10px; color: gray">
            <a :href="'#/store/' + currentbookstore.id" style="color: gray">{{
              currentbookstore.name
            }}</a>
          </el-col>
          ...
          <el-col style="padding: 10px; background-color: #f5f5f5">
            <p>44个商家在售</p>
            <span style="color: red">&yen;<strong>29.40</strong></span
            >&nbsp;起
            <p>商家预计送达时间5-14天</p>
          </el-col>
        </el-col>
      ...
      <el-row style="padding: 10px 8% 0 8%">
        <el-col :span="4">
          <p>浏览此商品的顾客也同时浏览</p>
          <el-col
            style="text-align: center; padding: 10px; border: 1px solid #f5f5f5"
            v-for="(item, index) in books_other"
            :key="index"
          >
            ...
            <el-col style="text-align: left; margin-top: 10px; padding-left: 10px">
              <span style="font-size: 16px; color: red; text-align: left"
                >&yen;{{ item.price }}</span
              >
            </el-col>
            <el-col
        ...
        <el-col :span="19" :offset="1">
          <el-tabs type="card">
            <el-tab-pane label="经常一起购买的商品">
              ...
              <el-row v-else style="padding: 10px; margin-bottom: 30px">
                <el-col
                  :span="5"
                  v-for="(item, index) in currentBook.relativebooks.data"
                  type="flex"
                  :key="index"
                >
                  ...
                  <span style="font-size: 18px; color: red"
                    >&yen;<strong>{{ item.price }}</strong></span
                  >
                  <el-link type="text" plain :underline="false">{{
                    item.book_name
                  }}</el-link>
                </el-col>
              </el-row>
          ...
          <el-tabs type="card" tab-position="top">
              ...
              <div>
                ...
                <el-row>
                  ...
                  <el-col :span="18">
                    ...
                    <el-row>
                      <el-tag
                        v-for="(item, index) in currentBook.comments.tags"
                        :key="index"
                        type="warning"
                        class="comments-tags"
              ...
              <div style="margin-top: 20px">
                <div class="comments-title">
                  ...
                  <el-col :span="2" :offset="2"
                    >好评 ({{ currentBook.comments.evaluate[0] }})</el-col
                  >
                  <el-col :span="2" :offset="2"
                    >中评 ({{ currentBook.comments.evaluate[1] }})</el-col
                  >
                  <el-col :span="2" :offset="2"
                    >差评 ({{ currentBook.comments.evaluate[2] }})</el-col
                  >
                  <el-col :span="2" :offset="2"
                    >有图 ({{ currentBook.comments.evaluate[3] }})</el-col
                  >
                </div>
                <ul class="comments-body">
                  <li
                    v-for="(item, index) in currentBook.comments.data"
                    style="width: 100%"
                  >
                    <el-col :span="19">
                      ...
                      <el-col style="margin-top: 10px">
                        <el-link :href="item.detail_url" plain :underline="false">{{
                          item.detail
                        }}</el-link>
                      </el-col>
                      <el-col style="margin-top: 5px">
                        ...
                        <img
                          src="http://product.dangdang.com/images/icon_yg.png"
                          alt=""
                        />
                        <span class="fr">
                          <el-button type="text" class="glyphicon glyphicon-thumbs-up">{{
                            item.approve
                          }}</el-button>
                        </span>
                      </el-col>
            ...
            <el-tab-pane :label="'商品问答(' + currentBook.answers.total + ')'"
              >123</el-tab-pane
            >
          </el-tabs>
        </el-col>
  ...
  data() {
    return {
      ...
      currentBook: {
        ...
        description:
          "樊登读书创始人樊登读书创始人洞察所有亲子问题背后的根源影响40，000,000人的领读人樊登在《读懂孩子的心》后，向家长们解答了家庭教育的底层逻辑。为什么在家长的眼中，孩子的问题是无穷无尽的？为什么家长一味给孩子找“药方”，却不能想想自己是不是可以改和弥补？焦虑和担心不能让孩子变得更好！樊登洞察所有亲子问题背后的根源：你必须成长，才能帮孩子成长。",
        price: {
          now: 18.99,
          before: 45.99,
        },
        ...
        info: {
          content:
            "樊登不仅是 40，000，000 会员的领读人，还是参与、陪伴和享受孩子成长的父亲。\n" +
          ...
          preface:
            "因为介绍了一些家庭教育方面的知识，很多家长便把我当成了家庭教育的专家，总喜欢问我关于孩子的问题：“我儿子不爱学习，该让他看什么书？”“我女儿自律性差，应该读什么书？ ”“我儿子做事缺乏耐心…… ”“ 我女儿脾气暴躁……”……\n" +
            "被问得多了，我就总结出了几条规律：\n" +
          ...
          data: [
            {
              score: 5,
              detail:
                "书收到了，还没有开始看，今年一直都在听樊登读书会的书，特别是教育类型的，感觉自己有事不是一个合格的妈妈！需要更深入的学习！争取做一个合格的妈妈！",
      ...
      books_other: [
        {
          ...
          title:
            "妈妈知道怎么办：一学就通的养育法入选樊登读书年度书单俞敏洪、马东、凯叔、王芳强力推荐 百万妈妈信赖，著名主持人，亲子教育专家王小骞全新力作！硬核带娃攻略，拯救父母的终极武器！赠送价值129元的《给孩子的大师艺术课》！",
